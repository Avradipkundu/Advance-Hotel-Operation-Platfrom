...
import { X, Calendar,UserCheck, Users, MapPin, Star, Calculator } from 'lucide-react';
...
const BookingModal = ({ isOpen, onClose, roomData }) => {
  ...
  const [availableMailds, setAvailableMailds] = useState([]);
  ...
  const fetchAvailableMaids = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:8000/api/availableStaffs', {
        headers: { Authorization: `Bearer ${token}` }
      });
      setAvailableMailds(response.data);
    } catch (error) {
      console.error('Failed to fetch available maids:', error);
      toast.error('Failed to load available maids');
    } finally {
  ...
  const handleBooking = async (e) => {
    ...
    try {
      ...
      const reservationData = {
        ...
        maidId: bookingData.maidId || null,
      ...
      const selectedMaid = availableMailds.find(maid => maid._id === bookingData.maidId);
      const maidInfo = selectedMaid ? `\nMaid: ${selectedMaid.name} `: '';

      alert(`Booking confirmed for ${roomData?.name}!${maidInfo}\nReservation ID: ${response.data._id}\nTotal: ₹${pricing.totalPrice} for ${pricing.nights} nights`);
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-all duration-300 ease-out ${isVisible ? 'opacity-100' : 'opacity-0'
        }`}
    >
      {/* Backdrop */}
      <div
        className={`absolute inset-0 backdrop-blur-sm transition-all duration-300 ease-out ${isVisible ? ' bg-opacity-50' : 'bg-opacity-0'
          }`}
        onClick={handleClose}
      />
      ...
      <div
        className={`relative bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto transition-all duration-300 ease-out transform ${isVisible
          ...
          <div className="p-4 sm:p-6">
            ...
            <form onSubmit={handleBooking} className="space-y-4">
                ...
                <div className="sm:col-span-2 lg:col-span-1">
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      disabled={maidsLoading}
                      className="w-full pl-10 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
                    >
                      <option value="">
                        {maidsLoading ? 'Loading maids...' : 'No maid service'}
                      </option>
                      {availableMailds.map(maid => (
                  ...
                  {maidsLoading && (
                    <p className="text-xs text-gray-500 mt-1">Loading available maids...</p>
                  )}
                </div>
                {/* Check-In */}
                <div>
              ...
              <button
                ...
                className={`w-full py-3 px-6 rounded-lg transition-colors duration-200 font-medium ${pricing.nights > 0 && !isLoading
